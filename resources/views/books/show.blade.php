                        ...
                        <div class="md:col-span-2 flex flex-col">
                            ...
                            <div class="flex space-x-4">
                                ...
                                <form action="{{ route('wishlist.toggle', $book) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="p-3 border {{ auth()->check() && auth()->user()->wishlist->contains($book->id) ? 'border-red-200 text-red-500 bg-red-50' : 'border-gray-300 text-gray-500 hover:bg-gray-50' }} rounded-lg transition duration-150 ease-in-out">
                                        <svg class="w-6 h-6 {{ auth()->check() && auth()->user()->wishlist->contains($book->id) ? 'fill-current' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                                    </button>
                                </form>
                            </div>
                        </div>
