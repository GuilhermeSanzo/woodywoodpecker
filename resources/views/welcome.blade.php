...
<x-app-layout>
    ...
    <div class="py-16 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-end mb-8">
                <div>
                    <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight">Most Popular Books</h2>
                    <p class="mt-2 text-lg text-gray-500 italic">The most visited titles in our collection.</p>
                </div>
                    ...
                    <div class="bg-white overflow-hidden shadow-lg rounded-xl flex flex-col border border-gray-100 transform hover:-translate-y-1 transition duration-300 relative">
                        ...
                        <div class="p-4 bg-white flex-grow">
                            ...
                            <div class="text-xs text-gray-500 flex items-center">
                                <span class="bg-gray-100 px-2 py-1 rounded">{{ $book->genre?->name ?? 'N/A' }}</span>
                            </div>
                        </div>
                        ...
                        <div class="p-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center relative z-10">
                            ...
                            <form action="{{ route('cart.add', $book) }}" method="POST">
                                @csrf
                                <button type="submit" class="p-2 {{ isset(session('cart', [])[$book->id]) ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-indigo-600 hover:bg-indigo-700' }} text-white rounded-full transition duration-150">
                                    @if(isset(session('cart', [])[$book->id]))
    ...
    <div class="py-16 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight mb-12 text-center underline decoration-indigo-500 decoration-4 underline-offset-8">Most Popular Authors</h2>
                    ...
                    <div class="flex items-center space-x-6 group">
                        <div class="flex-shrink-0">
                            <div class="relative">
                                ...
                                @if($author->image)
                                    <img class="h-24 w-24 rounded-full object-cover shadow-md border-2 border-white" src="{{ asset('storage/' . $author->image) }}" alt="{{ $author->name }}">
                                @else
                        ...
                        <div>
                            ...
                            <p class="text-indigo-600 text-sm font-semibold mb-2">{{ $author->pseudonym }}</p>
